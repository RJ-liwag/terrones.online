<?php
include("../conn.php");
include "functions/rooms/get-rooms.php";
...
<head>
	...
	<link rel="stylesheet" href="assets/css/demo.css">
	 <style>
        /* Hide the number input arrows in Chrome, Safari, Edge, and Opera */
...
<body>
		...
		<div class="main-panel">
			<div class="content">
				<div class="container-fluid">
					<div class="row mb-2">
						<h4 class="col page-title">Rooms</h4>
						<div class="col d-flex justify-content-end">
							<!-- <form class="d-flex mr-3">
								<input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
								<button class="btn btn-outline-success" type="submit">Search</button>
							</form> -->
							<button type="button" class="btn btn-success" data-toggle="modal" data-target="#roomModal"><i class="la la-plus">&nbsp;
								</i>Add</button>
									...
									<div class="card-header">
										<dix class="row">
											<div class="col container">
												<div class="row">
													<input type="hidden" value="<?php echo $row['room_id']; ?>" id="room_id" name="room_id">
													<div class="col-sm-2">
														<img class="img-thumbnail w-100 h-100" src="assets/rooms-cottages/<?php echo $row['room_img'] ?>" alt="" ;>
													</div>
													<div class="col-sm-10">
														<div class="row">
											...
											<div class="col-4 d-flex align-items-end flex-column">
												<div>
													<button class="btn btn-danger delete_room_btn" data-id="<?php echo $row['room_id']; ?>">
														<i class="fa-solid fa-ban"></i>
													</button>

													<button class="btn btn-warning" onclick="open_modal_user_view(<?php echo $row['room_id'] ?>)"><i class="la la-pencil-square-o"></i></button>
												</div>
											</div>
										</dix>
									</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h6 class="modal-title" id="exampleModalLabel">Add Room</h6>
					<!-- <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button> -->
				</div>
				<div class="modal-body">
					<div>
						<div class="mb-2 d-flex justify-content-center">
							<img id="selectedImage" src="https://jjgf.com/images/defaultx2.jpg" alt="example placeholder" style="width: 200px;" />
						</div>
						<div class="d-flex justify-content-center">
							<div data-mdb-ripple-init class="btn btn-primary btn-rounded">
								<label class="form-label text-white m-1" for="room_img">Choose Photo</label>
								<input type="file" class="form-control d-none" id="room_img" name="room_img" accept=".jpeg, .jpg, .png" onchange="displaySelectedImage(event, 'selectedImage')" />
							</div>
						</div>
						<div class="row">
							<div class="col">
						...
						<div class="row" style="width:53%;">
							<div class="col">
								...
								<input type="number" class="form-control" id="max_capacity" name="max_capacity" autocomplete="off">
							</div>
						</div>
	...
	<div id="user-update" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<div class="modal-header bg-success">
					<h4 class="modal-title text-white" id="standard-modalLabel">Edit Rooms</h4>
...
<script>
	$(document).on('click', '.delete_room_btn', function(e) {
		...
		Swal.fire({
			title: 'Are you sure you want to set this room inactive?',
			icon: 'question',
			...
			if (result.isConfirmed) {
				$.ajax({
					type: 'POST',
					url: 'functions/rooms/delete-room.php',
					data: {
						room_id: room_id
					},
...
<script>
	$(document).on('click', '.btn_room_add', function(e) {
		...
		if (room_img.files.length > 0) {
			...
			isValid = false;
			showValidationError("#room_img", "An image is required.");
		}
		...
		$.ajax({
			type: 'POST',
			url: 'functions/rooms/add-room.php',
			data: formData,
			processData: false,
			contentType: false,
			success: function(data) {
...
<script>
	function open_modal_user_view(room_id) {
		...
		$.post(
			"functions/rooms/view-edit-rooms.php", {
				room_id: room_id,
			},
			function(data) {

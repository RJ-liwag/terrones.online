...
$sql = $mysqli->prepare("SELECT * FROM cottages WHERE room_id = ?");
	...
	<div>
		...
		<div class="row">
			<div class="col">
				...
				<input type="text" class="form-control" id="edit_cottages_name" name="edit_cottages_name"
...
<script>
	...
	$(document).on('click', '.btn_room_edit', function(e) {
		...
		addField("room_name", "#edit_cottages_name", true); // Required field
		...
		// Validate and add image (optional)
		...
		$.ajax({
			type: 'POST',
			url: 'functions/cottages/update-cottages.php',
			data: formData,
			processData: false,
			contentType: false,
